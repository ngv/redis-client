...
Redis.prototype.connect = function() {
  ...
  if (this._conn) {
      this.quit(); //gracefully close an existing connection
  }
...
function createCommandSender(commandName) {
  return function() {
    ...
    try {
        ...
        //eg. the socket got closed on us by a timeout on the redis server,
        //so reset the connection so this redis instance can still be used
        this.fatal('Error occured processing cmd:'+e);
    }
...
function processResponse(self) {
  ...
  if ('-+$*:'.indexOf(typePrefix) < 0) {
      var reader = new BufferedReader(
                      new InputStreamReader(responseStream, Charset.forName("UTF-8")));
      self.fatal("invalid Redis type Prefix:"+typePrefix+
                      " responseStream:"+reader.readLine());
  }
...
exports.Redis.prototype.sort = function(key, options) {
  ...
  try {
      ...
      //so reset the connection so this redis instance can still be used
      this.fatal('Error during dort command:'+e);
  }
