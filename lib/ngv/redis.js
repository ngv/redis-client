...
Redis.prototype.connect = function() {
  ...
  this._inp = this._conn.getInputStream();
...
Redis.prototype.quit = function() {
  ...
  log.debug('> quit');
  var utfWriter = new OutputStreamWriter(this._out, Charset.forName("UTF-8"));
  utfWriter.write('quit' + CRLF);
  this._conn.close();
  this._conn = null;
};
...
function createCommandSender(commandName) {
  return function() {
    ...
    try {
        ...
        //eg. the socket got closedon us by a timeout on the redis server,
        //so reset the connection so this redis instance can still be used
        this.quit();
        throw e;
    }
 ...
 */
function processResponse(responseStream, currentCmd) {

  try {
      // Read first char of Redis response to find out type of response?
      var typePrefix = String.fromCharCode(responseStream.read());
  } catch(e) {
      this.fatal("Error during responseStream.read"+e);

  }
  if ('-+$*:'.indexOf(typePrefix) < 0) {
      var reader = new BufferedReader(new InputStreamReader(responseStream, Charset.forName("UTF-8")));
      this.fatal("invalid Redis type Prefix:"+typePrefix+" responseStream:"+responseStream.readLine());
  }
  log.debug('<type char:'+typePrefix);

  var handler = replyPrefixToHandler[typePrefix];

  var reader = new InputStreamReader(responseStream, Charset.forName("UTF-8"));
  return postProcessResults(handler(responseStream), currentCmd);
}
...
function handleBulkReply(responseStream) {
  ...
  var count = responseStream.read(buf);
  responseStream.skip(2);//read out trailing CR+LF

  var s = String(java.lang.String(buf, Charset.forName("UTF-8")));
