...
Redis.prototype.quit = function() {
  ...
  log.debug('> quit');
  var writer = new OutputStreamWriter(this._out, Charset.forName("UTF-8"));
  writeCmd(this, writer, 'quit');
  this._conn.close();
  this._conn = null;
};
 ...
 * @returns
 */
function writeCmd(self, writer, cmd) {
	try {
        writer.write(cmd);
        writer.flush();
    } catch(e) {
...
function processResponse(self) {
  ...
  try {
	  var byte = responseStream.read();
	  if (byte == -1) { //redis timed our conn out for being Idle!
		  log.error("redis -1 "+self.currentCmd);
		  throw new Error("redis timeout");
	  }
      // Read first char of Redis response to find out type of response?
      var typePrefix = String.fromCharCode(byte);
  } catch(e) {
